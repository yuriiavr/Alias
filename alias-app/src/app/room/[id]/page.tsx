...
const newPlayer = { name: userName, team: isSpec ? -1 : team, isSpectator: isSpec, isCreator: creatorStatus };

localStorage.setItem(`alias-session-${roomId}`, JSON.stringify({ 
  userName, userTeam: team, isSpectator: isSpec, isCreator: creatorStatus
}));
      ...
      <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
          ...
          {currentRound === 1 && usedWords.length === 0 ? (
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                {teams.map((team, idx) => (
                  <input
                    ...
                    className={`w-full px-5 py-4 bg-black/40 border border-zinc-800 rounded-2xl focus:border-red-800/50 transition-all outline-none text-white font-bold ${!isCreator && 'opacity-50 cursor-not-allowed'}`}
                  />
                ))}
              </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {["easy", "mixed", "medium", "hard"].map((lvl) => (
                    <button
                      ...
                      className={`py-3 rounded-xl text-[9px] uppercase font-black transition-all border ${difficulty === lvl ? "bg-zinc-100 text-black" : "bg-transparent text-zinc-600 border-zinc-800"} ${!isCreator && 'opacity-50'}`}
                    >
                      {lvl}
                    </button>
                  ))}
                ...
                <div className="flex gap-2">
                  {[3, 5, 10].map((num) => (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-xl text-xs font-bold border ${totalRounds === num ? "bg-white text-black" : "border-zinc-800 text-zinc-600"} ${!isCreator && 'opacity-50'}`}
                    >
                      {num}
                    </button>
                  ))}
              ...
              <div className="pt-4 border-t border-zinc-800/50">
                 <label className="text-[9px] uppercase tracking-[0.2em] text-zinc-500 font-black ml-1 block mb-3">Гравці у лобі</label>
                 <div className="flex flex-wrap gap-2">
                    {players.map((p, i) => (
                      <div key={i} className="px-3 py-1.5 bg-zinc-800/50 rounded-lg border border-zinc-700/50 flex items-center gap-2">
                        <span className={`w-1.5 h-1.5 rounded-full ${p.team === 0 ? 'bg-blue-500' : p.team === 1 ? 'bg-red-500' : 'bg-zinc-500'}`}></span>
                        <span className="text-[10px] font-bold text-white uppercase">{p.name} {p.isCreator && '👑'}</span>
                      </div>
                    ))}
                 </div>
              </div>
        ...
        {isCreator || (!isSpectator && userTeam === currentTeamIndex) ? (
          ...
          <div className="text-center p-6 border border-dashed border-zinc-800 rounded-2xl opacity-50">
            <p className="text-[10px] uppercase font-bold tracking-widest">Чекаємо на адміна або команду {teams[currentTeamIndex].name}...</p>
          </div>
        )}
      </div>
    )}
